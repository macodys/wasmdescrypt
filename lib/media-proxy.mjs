...
let proxyCacheReady = false;

function resolveStormProxyUrl() {
  const value = process.env.STORM_PROXY_URL || process.env.HTTPS_PROXY || "";
...
function normalizeProxyUrl(raw) {
  ...
  if (/^(https?|socks4|socks5|socks):\/\//i.test(value)) {
    return value;
  }
  return `http://${value}`;
}

function getStormProxyAgent() {
  if (proxyCacheReady) {
    return cachedProxyAgent;
  }

  proxyCacheReady = true;
  const rawUrl = resolveStormProxyUrl();
...
export async function getStormProxyStatus() {
  const rawUrl = resolveStormProxyUrl();
  if (!rawUrl) {
  ...
  try {
    const agent = getStormProxyAgent();
    if (!agent) {
      return { configured: true, active: false, raw: rawUrl, error: "Invalid proxy URL format" };
    }

    let reachable = false;
    let probeError = null;
    const testUrl = "https://megacloud.live/";
    try {
      const probe = await undiciFetch(testUrl, { dispatcher: agent, method: "HEAD" });
      reachable = probe.ok || probe.status < 500;
    } catch (err) {
      probeError = err.cause?.message || err.message || "Connection failed";
    }

    return { configured: true, active: Boolean(agent), raw: rawUrl, reachable, probeError };
  } catch (error) {
    return { configured: true, active: false, raw: rawUrl, error: error.message };
  }
}

export async function fetchUpstream(targetUrl, init = {}) {
  const headers = init.headers || buildUpstreamHeaders(targetUrl);
  const agent = getStormProxyAgent();

  // Try origin bypass FIRST — hits vod3.ironwallnet.com directly, no Cloudflare
  const originAttempt = toOriginUrl(targetUrl);
  if (originAttempt) {
    const originUrls = [originAttempt.url];
    if (originAttempt.strippedUrl) {
      originUrls.push(originAttempt.strippedUrl);
    }
    for (const originUrl of originUrls) {
      try {
        const originResponse = await fetch(originUrl, {
          ...init,
          headers: { ...headers, ...originAttempt.headers },
        });
        if (originResponse.ok) {
          return originResponse;
        }
      } catch {
        // try next origin URL or fall through to storm
      }
    }
  }

  if (agent) {
    return undiciFetch(targetUrl, { ...init, headers, dispatcher: agent });
  }
...
/**
 * Extract the origin host from a Storm URL's `host` query parameter.
 * e.g. `host=https://vod3.ironwallnet.com:6069` → `vod3.ironwallnet.com:6069`
 */
export function extractOriginHost(url) {
  try {
    const parsed = new URL(url);
    const hostParam = parsed.searchParams.get("host");
    if (hostParam) {
      const hostUrl = new URL(hostParam.startsWith("http") ? hostParam : `https://${hostParam}`);
      return hostUrl.host; // e.g. "vod3.ironwallnet.com:6069"
    }
  } catch {
    // ignore
  }
...
/**
 * Convert a Storm `vodvidl.site` URL to a direct origin URL by swapping
 * the hostname with the `host` query parameter value, preserving path + query.
 * Tries multiple path variations since Storm may rewrite paths internally.
 */
export function toOriginUrl(stormUrl) {
  const originHost = extractOriginHost(stormUrl);
  if (!originHost) {
    return null;
  }

  try {
    const parsed = new URL(stormUrl);
    // Only works for vodvidl.site URLs
    ...
    function buildUrl(pathname) {
      ...
      copy.hostname = originHostname;
      copy.port = originPort || (parsed.protocol === "https:" ? "443" : "80");
    ...
    // 1. Full original path (e.g. /proxy/wiwii/HASH/1080/index.m3u8)
    // 2. Strip /proxy/wiwii/HASH prefix to the content path
    const originalPath = parsed.pathname;
    const pathMatch = originalPath.match(/\/proxy\/wiwii\/[a-f0-9]+\/(.+)/);
    const strippedPath = pathMatch ? `/${pathMatch[1]}` : null;

    const result = {
      url: buildUrl(originalPath),
      strippedUrl: strippedPath ? buildUrl(strippedPath) : null,
      headers: {
        Host: originHost,
...
export async function fetchStormManifest(upstreamUrl, proxyBase) {
  const headers = buildUpstreamHeaders(upstreamUrl);
  const viaProxy = Boolean(getStormProxyAgent());
  const originAttempt = toOriginUrl(upstreamUrl);

  // Try origin direct first (bypass Storm's Cloudflare entirely)
  if (originAttempt) {
    try {
      const originResponse = await fetch(originAttempt.url, {
        headers: { ...headers, ...originAttempt.headers },
      });
      if (originResponse.ok) {
        const text = await originResponse.text();
        if (text.trimStart().startsWith("#EXTM3U")) {
          const rewritten = proxyBase ? rewriteM3u8(text, originAttempt.url, proxyBase) : text;
          return { ok: true, status: originResponse.status, viaProxy: false, viaOrigin: true, text: rewritten };
        }
      }
    } catch {
      // Origin direct failed, fall through to storm
    }
  }

  try {
    const response = await fetchUpstream(upstreamUrl, { headers });
    ...
    if (!text.trimStart().startsWith("#EXTM3U")) {
      return {
        ...
        viaProxy,
        viaOrigin: false,
        error: "Response was not a valid m3u8",
      };
    }

    const rewritten = proxyBase ? rewriteM3u8(text, upstreamUrl, proxyBase) : text;
    return { ok: true, status, viaProxy, viaOrigin: false, text: rewritten };
  } catch (error) {
    return {
      ok: false,
      status: 0,
...
export async function proxyMediaRequest(targetUrl, hdrParam, incomingHeaders, writeResponse) {
  ...
  const response = await fetchUpstream(targetUrl, { headers: upstreamHeaders });
...
export function applyProxyToLinkResponse(body, { baseOrigin, proxy, streamHeaders }) {
  ...
  if (stormHls) {
    ...
    return {
      ...
      proxyMode: "client",
      playbackHint:
        "Do NOT paste storm.vodvidl.site links in VLC. Paste the entire url/vlcMrl value here, open playerUrl in a browser, or download downloadUrl and open the .m3u file in VLC (Media → Open File).",
    };
  }
  ...
  if (body.streamType === "hls") {
    proxied.playbackHint =
      "Open playerUrl in a browser. MP4-only external players cannot play Storm HLS; use downloadUrl for VLC instead.";
  }
