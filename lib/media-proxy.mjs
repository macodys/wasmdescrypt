...
export function applyProxyToLinkResponse(body, { baseOrigin, proxy, streamHeaders }) {
  ...
  if (body.streamType === "hls") {
    proxied.playbackHint =
      "Use the proxied url field. Storm links expire after a few hours — regenerate via /api/link.";
    if (body.rawUrl || body.url) {
      const upstream = body.rawUrl || body.url;
      proxied.vlcMrl = `:http-referrer=https://megacloud.live/ ${upstream}`;
    }
  }
