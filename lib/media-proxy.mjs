...
export function buildStormVlcMrl(upstreamUrl) {
  ...
  return `:http-user-agent=${userAgent} :http-referrer=${referer} ${upstreamUrl}`;
}
...
export function applyProxyToLinkResponse(body, { baseOrigin, proxy, streamHeaders }) {
  ...
  if (stormHls) {
    ...
    return {
      ...
      proxyMode: "client",
      playbackHint:
        "Storm HLS cannot play from a plain URL in VLC. Download /api/vlc?... (recommended) or paste vlcMrl into Media → Open Network Stream. Links expire after a few hours.",
    };
  }
