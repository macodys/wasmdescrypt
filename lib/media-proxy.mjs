...
function getStormProxyAgent(returnErrors) {
  if (proxyCacheReady) {
  ...
  if (proxyUrl) {
    try {
      ...
      throw new Error(
        `Invalid STORM_PROXY_URL (resolved to "${proxyUrl}", raw="${rawUrl}"): ${error.message}`
      );
    }
  }
  return cachedProxyAgent;
...
export function getStormProxyStatus() {
  const rawUrl = resolveStormProxyUrl();
  if (!rawUrl) {
  ...
  try {
    getStormProxyAgent();
    return { configured: true, active: true, raw: rawUrl };
  } catch (error) {
