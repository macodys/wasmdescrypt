...
export function buildStormVlcMrl(upstreamUrl) {
  ...
  return `:http-referrer=${referer} ${upstreamUrl}`;
}
...
export function applyProxyToLinkResponse(body, { baseOrigin, proxy, streamHeaders }) {
  ...
  if (stormHls) {
    ...
    return {
      ...
      proxyMode: "client",
      playbackHint:
        "Storm HLS requires Referer headers. In VLC use Media → Open Network Stream and paste vlcMrl (not the plain url). Links expire after a few hours — regenerate via /api/link.",
    };
  }
