let stormProxyDispatcher;

async function getStormProxyDispatcher() {
  const proxyUrl = process.env.STORM_PROXY_URL || process.env.HTTPS_PROXY;
  if (!proxyUrl) {
    return undefined;
  }

  if (stormProxyDispatcher !== undefined) {
    return stormProxyDispatcher || undefined;
  }

  try {
    const { ProxyAgent } = await import("undici");
    stormProxyDispatcher = new ProxyAgent(proxyUrl);
  } catch {
    stormProxyDispatcher = null;
  }
...
export async function fetchUpstream(targetUrl, init = {}) {
  ...
  const dispatcher = await getStormProxyDispatcher();
