...
export async function fetchUpstream(targetUrl, init = {}) {
  ...
  // Try origin bypass for Storm URLs (skip Cloudflare), but NOT if we have a proxy
  if (!agent) {
 ...
 * Also sets the `Host` header so the origin responds correctly.
 */
export function toOriginUrl(stormUrl) {
  ...
  try {
    ...
    // Swap the hostname to the origin
    parsed.hostname = originHost.split(":")[0];
    const port = originHost.split(":")[1];
    if (port) {
      parsed.port = port;
    } else {
      parsed.port = ""; // Use default
    }
    // Remove the host param since we're now using it as the destination
    parsed.searchParams.delete("host");

    // Return the rewritten URL + headers that the caller should set
    return {
      url: parsed.toString(),
      headers: {
        Host: originHost,
