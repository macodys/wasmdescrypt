import { extractStormRequestHeaders, isStormUrl } from "./storm-hls.mjs";
...
function enrichPlayerUrls(body, baseOrigin) {
  if (!baseOrigin) {
    return body;
  }
  ...
  if (body.streamType === "hls") {
    ...
    const upstream = body.streamQualityUrl || body.qualityUrl || body.url;
    if (isStormUrl(upstream)) {
      enriched.plugin = {
        m3u8: upstream,
        headers: extractStormRequestHeaders(upstream),
      };
      enriched.playbackHint =
        "Use plugin.m3u8 with plugin.headers in external players. For browser playback open playerUrl. For VLC use downloadUrl.";
    }
...
function finalizeResponse(body, query, options) {
  ...
  return enrichPlayerUrls(proxied, options.baseOrigin);
}
