...
async function enrichPlayerUrls(body, baseOrigin) {
  ...
  if (body.streamType === "hls") {
    ...
    enriched.manifestEdgeUrl = `${baseOrigin}/api/manifest-edge?${playerQuery.toString()}`;

    const upstream = body.streamQualityUrl || body.rawQualityUrl || body.rawUrl;
    if (upstream && isStormUrl(upstream)) {
      const originInfo = toOriginUrl(upstream);
      enriched.plugin = {
        m3u8: upstream,
        headers: extractStormRequestHeaders(upstream),
      };
      if (originInfo) {
        enriched.originUrl = originInfo.url;
        enriched.originHeaders = originInfo.headers;
      }
      enriched.playbackHint =
        "Browser: open playerUrl. External: plugin.m3u8 + plugin.headers (FetchV). VLC: downloadUrl.";
    }
...
async function finalizeResponse(body, query, options) {
  ...
  if (enriched.streamType === "hls" && options.baseOrigin) {
    ...
    if (upstream && isStormUrl(upstream)) {
      ...
      // Also add the origin-bypass URL
      const originAttempt = toOriginUrl(upstream);
      if (originAttempt) {
        enriched.originBypass = {
          url: originAttempt.url,
          strippedUrl: originAttempt.strippedUrl,
          headers: originAttempt.headers,
        };
      }
    }
  }
